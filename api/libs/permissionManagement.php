...
class Permissions {
	 ...
	 */
	public function set_channel_permissions($channel, $userid, $isAdmin = false) {
		global $mysqli;

		// has to be channel admin
		if (!$this->_user->has_privilege($channel, ADMIN)) {
			return false;
		}
	...
	public function list_channel_permissions($channel) {
		...
		$sql = 'SELECT a.userid, us.name, a.level FROM ChannelAccess a
                ...
                WHERE a.channelid = ? ORDER BY us.name';
		$stmt = $mysqli->prepare($sql);
	 ...
	 */
	public function set_unit_permissions($unit, $userid, $isAdmin = false) {
		global $mysqli;

		// has to be unit admin
		if (!$this->_user->has_privilege($unit, ADMIN, true)) {
			return false;
		}
	...
	public function list_unit_permissions($unit) {
		...
		if (!$this->_user->has_privilege($unit, ADMIN)) {
			return array();
		}
